-- ============================================================
-- common-module 의 Inbox / Outbox 테이블
...
--
-- event_type 컬럼이 Kafka 토픽으로 직접 사용되며,
-- OutboxRelayScheduler 가 PENDING/FAILED 레코드를 재시도한다.
CREATE TABLE match.p_outbox (
...
CREATE INDEX idx_outbox_status ON match.p_outbox (status);


-- ── p_inbox (멱등 소비자 / Idempotent Consumer) ───────────────
-- Inbox 는 BaseEntity 를 상속하지 않는다. processed_at 만 보존(@CreatedDate).
CREATE TABLE match.p_inbox (
    message_id      UUID            NOT NULL PRIMARY KEY,
    message_group   VARCHAR(50),
    processed_at    TIMESTAMP       NOT NULL
);

CREATE INDEX idx_inbox_message_group ON match.p_inbox (message_group);
CREATE INDEX idx_inbox_processed_at  ON match.p_inbox (processed_at);
